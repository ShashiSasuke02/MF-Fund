...
export const calculateNextPaymentDate = (currentDate, frequency) => {
    ...
    switch (frequency) {
        ...
        case 'QUARTERLY': {
            next = new Date(current);
            const targetMonth = (next.getMonth() + 3) % 12;
            next.setMonth(next.getMonth() + 3);
            // Check for month skip logic
            if (next.getMonth() !== targetMonth) {
                next.setDate(0);
            }
            break;
        ...
        case 'YEARLY': {
            next = new Date(current);
            const targetMonth = next.getMonth(); // Should be same month
            next.setFullYear(current.getFullYear() + 1);
            // Check for leap year edge case (Feb 29 -> Feb 28)
            if (next.getMonth() !== targetMonth) {
                next.setDate(0);
            }
            break;
