...
export default function PPFCalculator({ interestRates }) {
  const defaultRate = interestRates?.ppf || 7.1;
  ...
  const handleCalculate = async (e) => {
    ...
    try {
      ...
      const response = await calculatorApi.calculatePPF({
    ...
    <div className="space-y-6">
      <div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-r">
        <div className="flex">
          ...
          <div>
            <h3 className="text-sm font-semibold text-blue-900 mb-1">About PPF</h3>
            <p className="text-sm text-blue-800">15-year lock-in, annual compounding, Section 80C eligible. Default rate {defaultRate}% p.a.</p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleCalculate} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2" htmlFor="tenureYears">Deposit Period (years) *</label>
            <input
              id="tenureYears"
              name="tenureYears"
              type="number"
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          >
            {loading ? <LoadingSpinner size="small" /> : 'Calculate PPF Returns'}
          </button>
          <button
            type="button"
            onClick={handleReset}
        ...
        <div className="mt-8 bg-gradient-to-br from-emerald-50 to-teal-50 rounded-xl p-6 border-2 border-emerald-200">
          ...
          <div className="bg-white rounded-lg p-4 shadow-md grid grid-cols-2 gap-4 text-sm">
            ...
            <div>
              ...
              <p className="font-semibold text-emerald-700">{(result.maturityAmount / result.totalDeposit).toFixed(2)}x</p>
            </div>
          </div>

          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            <p className="text-sm text-gray-700"><strong>Tax Benefit:</strong> 80C eligible (up to ₹1.5L). Interest and maturity are tax-free.</p>
          </div>
        </div>
      )}
    </div>
