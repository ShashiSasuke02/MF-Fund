...
/**
 * Fixed Deposit Interest Payout Calculator
 */
export default function FDPayoutCalculator({ interestRates }) {
	const defaultRate = interestRates?.fixedDeposit?.['1year'] || 6.5;
	...
	const handleCalculate = async (e) => {
		...
		try {
			...
			const response = await calculatorApi.calculateFDPayout({
		...
		<div className="space-y-6">
			...
			<div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-r">
				<div className="flex">
					...
					<div>
						<h3 className="text-sm font-semibold text-blue-900 mb-1">About Interest Payout FD</h3>
						<p className="text-sm text-blue-800">Earn regular interest payouts monthly/quarterly while principal stays locked till maturity.</p>
					</div>
				</div>
			</div>
			...
			<form onSubmit={handleCalculate} className="space-y-6">
				<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
					...
					<div>
						<label className="block text-sm font-semibold text-gray-700 mb-2" htmlFor="tenureMonths">Tenure (months) *</label>
						<input
							id="tenureMonths"
							name="tenureMonths"
							type="number"
					...
					<div>
						<label className="block text-sm font-semibold text-gray-700 mb-2" htmlFor="payoutFrequency">Payout Frequency</label>
						<select
							id="payoutFrequency"
				...
				<div className="flex flex-col sm:flex-row gap-3">
					...
					>
						{loading ? <LoadingSpinner size="small" /> : 'Calculate Interest Payout'}
					</button>
					<button
						type="button"
						onClick={handleReset}
				...
				<div className="mt-8 bg-gradient-to-br from-emerald-50 to-teal-50 rounded-xl p-6 border-2 border-emerald-200">
					...
					<div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
						<div className="bg-white rounded-lg p-4 shadow-md">
							<p className="text-sm text-gray-600 mb-1">Interest per Payout</p>
							<p className="text-2xl font-bold text-emerald-600">{formatCurrency(result.interestPerPayout)}</p>
						</div>
						<div className="bg-white rounded-lg p-4 shadow-md">
							<p className="text-sm text-gray-600 mb-1">Total Payouts</p>
							<p className="text-2xl font-bold text-blue-600">{result.numberOfPayouts}</p>
						</div>
						<div className="bg-white rounded-lg p-4 shadow-md">
							<p className="text-sm text-gray-600 mb-1">Total Interest</p>
							<p className="text-2xl font-bold text-purple-600">{formatCurrency(result.totalInterest)}</p>
						</div>
					</div>
					...
					<div className="bg-white rounded-lg p-4 shadow-md grid grid-cols-2 gap-4 text-sm">
						...
						<div>
							...
							<p className="font-semibold">{formatCurrency(result.maturityAmount)}</p>
						</div>
						<div>
						...
						<div>
							<p className="text-gray-600">Payout Frequency</p>
							<p className="font-semibold capitalize">{result.payoutFrequency}</p>
						</div>
					</div>
				</div>
			)}
