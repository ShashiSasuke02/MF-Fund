 ...
 */
export default function SCSSCalculator({ interestRates }) {
	const defaultRate = interestRates?.scss || 8.2;
	...
	const handleCalculate = async (e) => {
		...
		try {
			...
			const response = await calculatorApi.calculateSCSS({
			...
			setError(err.message || 'Failed to calculate SCSS');
		} finally {
			setLoading(false);
		}
	};
		...
		<div className="space-y-6">
			...
			<form onSubmit={handleCalculate} className="space-y-6">
				...
				<div className="flex flex-col sm:flex-row gap-3">
					...
					>
						{loading ? <LoadingSpinner size="small" /> : 'Calculate SCSS'}
					</button>
					<button
						type="button"
						onClick={handleReset}
				...
				<div className="mt-8 bg-gradient-to-br from-emerald-50 to-teal-50 rounded-xl p-6 border-2 border-emerald-200">
					...
					<div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
						<div className="bg-white rounded-lg p-4 shadow-md">
							<p className="text-sm text-gray-600 mb-1">Quarterly Interest</p>
							<p className="text-2xl font-bold text-emerald-600">{formatCurrency(result.quarterlyInterest)}</p>
						</div>
						<div className="bg-white rounded-lg p-4 shadow-md">
					...
					<div className="bg-white rounded-lg p-4 shadow-md grid grid-cols-2 gap-4 text-sm">
						...
						<div>
							<p className="text-gray-600">Quarters</p>
							<p className="font-semibold">{result.totalQuarters} payouts</p>
						</div>
					</div>
				</div>
			)}
