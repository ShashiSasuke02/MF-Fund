...
/**
 * Fixed Deposit (Cumulative) Calculator
 */
export default function FDCumulativeCalculator({ interestRates }) {
	const defaultRate = interestRates?.fixedDeposit?.['5year'] || 7.25;
	...
	const handleCalculate = async (e) => {
		...
		try {
			...
			const response = await calculatorApi.calculateFDCumulative({
			...
			setError(err.message || 'Failed to calculate FD maturity');
		} finally {
			setLoading(false);
		}
	};
		...
		<div className="space-y-6">
			<div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-r">
				<div className="flex">
					...
					<div>
						<h3 className="text-sm font-semibold text-blue-900 mb-1">About Cumulative FD</h3>
						<p className="text-sm text-blue-800">Interest is compounded and paid at maturity. Choose compounding frequency (quarterly default).</p>
					</div>
				</div>
			</div>
			...
			<form onSubmit={handleCalculate} className="space-y-6">
				<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
					...
					<div>
						<label className="block text-sm font-semibold text-gray-700 mb-2" htmlFor="compoundingFrequency">Compounding</label>
						<select
				...
				<div className="flex flex-col sm:flex-row gap-3">
					...
					>
						{loading ? <LoadingSpinner size="small" /> : 'Calculate Maturity'}
					</button>
					<button
						type="button"
						onClick={handleReset}
