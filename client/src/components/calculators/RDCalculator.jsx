 ...
 */
export default function RDCalculator({ interestRates }) {
	const defaultRate = interestRates?.recurringDeposit || 6.5;
	...
	const handleCalculate = async (e) => {
		...
		try {
			...
			const response = await calculatorApi.calculateRD({
			...
			setError(err.message || 'Failed to calculate RD');
		} finally {
			setLoading(false);
		}
	};
		...
		<div className="space-y-6">
			...
			<div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-r">
				<div className="flex">
					...
					<div>
						<h3 className="text-sm font-semibold text-blue-900 mb-1">About RD</h3>
						<p className="text-sm text-blue-800">Monthly deposits grow with quarterly compounding. Default tenure 60 months.</p>
					</div>
				</div>
			</div>
			...
			<form onSubmit={handleCalculate} className="space-y-6">
				...
				<div className="flex flex-col sm:flex-row gap-3">
					...
					>
						{loading ? <LoadingSpinner size="small" /> : 'Calculate RD Maturity'}
					</button>
					<button
						type="button"
						onClick={handleReset}
			...
			{result && (
				<div className="mt-8 bg-gradient-to-br from-emerald-50 to-teal-50 rounded-xl p-6 border-2 border-emerald-200">
					...
					<div className="bg-white rounded-lg p-4 shadow-md grid grid-cols-2 gap-4 text-sm">
						<div>
							<p className="text-gray-600">Monthly Deposit</p>
							<p className="font-semibold">{formatCurrency(result.monthlyDeposit)}</p>
						</div>
						<div>
