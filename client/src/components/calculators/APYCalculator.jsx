...
export default function APYCalculator() {
	...
	const handleCalculate = async (e) => {
		...
		try {
			...
			const response = await calculatorApi.calculateAPY({
				currentAge: parseInt(formData.currentAge, 10),
				pensionAmount: parseInt(formData.pensionAmount, 10)
			});

			setResult(response.data.result);
		} catch (err) {
			setError(err.message || 'Failed to calculate APY');
		} finally {
			setLoading(false);
		}
	};
	...
	const handleReset = () => {
		setFormData({ currentAge: '', pensionAmount: '5000' });
		setResult(null);
		setError(null);
	};
		...
		<div className="space-y-6">
			<div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-r">
				<div className="flex">
					...
					<div>
						<h3 className="text-sm font-semibold text-blue-900 mb-1">About APY</h3>
						<p className="text-sm text-blue-800">Government pension scheme for unorganized sector. Choose desired pension, pay till age 60.</p>
					</div>
				</div>
			</div>
			...
			<form onSubmit={handleCalculate} className="space-y-6">
				<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
					...
					<div>
						<label className="block text-sm font-semibold text-gray-700 mb-2" htmlFor="pensionAmount">Desired Monthly Pension *</label>
						<select
							id="pensionAmount"
							name="pensionAmount"
				...
				<div className="flex flex-col sm:flex-row gap-3">
					...
					>
						{loading ? <LoadingSpinner size="small" /> : 'Calculate APY Contribution'}
					</button>
					<button
						type="button"
						onClick={handleReset}
			...
			{result && (
				<div className="mt-8 bg-gradient-to-br from-emerald-50 to-teal-50 rounded-xl p-6 border-2 border-emerald-200">
					<h3 className="text-2xl font-bold text-gray-900 mb-6 flex items-center">
						...
						APY Contribution Summary
					</h3>

					<div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
						<div className="bg-white rounded-lg p-4 shadow-md">
							<p className="text-sm text-gray-600 mb-1">Monthly Contribution</p>
							<p className="text-2xl font-bold text-emerald-600">{formatCurrency(result.monthlyContribution)}</p>
						</div>
						<div className="bg-white rounded-lg p-4 shadow-md">
							<p className="text-sm text-gray-600 mb-1">Contribution Years</p>
							<p className="text-2xl font-bold text-blue-600">{result.contributionYears}</p>
						</div>
						<div className="bg-white rounded-lg p-4 shadow-md">
							<p className="text-sm text-gray-600 mb-1">Total Contribution</p>
							<p className="text-2xl font-bold text-purple-600">{formatCurrency(result.totalContribution)}</p>
						</div>
					</div>

					<div className="bg-white rounded-lg p-4 shadow-md grid grid-cols-2 gap-4 text-sm">
						<div>
							<p className="text-gray-600">Desired Pension</p>
							<p className="font-semibold">{formatCurrency(result.pensionAmount)}</p>
						</div>
						<div>
							<p className="text-gray-600">Retirement Age</p>
							<p className="font-semibold">{result.retirementAge} years</p>
						</div>
					</div>
				</div>
			)}
