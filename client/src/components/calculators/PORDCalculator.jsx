 ...
 */
export default function PORDCalculator({ interestRates }) {
	const defaultRate = interestRates?.poRD || 6.7;
	...
	const handleCalculate = async (e) => {
		...
		try {
			...
			const response = await calculatorApi.calculatePORD({
			...
			setError(err.message || 'Failed to calculate Post Office RD');
		} finally {
			setLoading(false);
		}
	};
		...
		<div className="space-y-6">
			...
			<div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-r">
				<div className="flex">
					...
					<div>
						<h3 className="text-sm font-semibold text-blue-900 mb-1">About Post Office RD</h3>
						<p className="text-sm text-blue-800">5-year recurring deposit with quarterly compounding.</p>
					</div>
				</div>
			</div>
			...
			<form onSubmit={handleCalculate} className="space-y-6">
				...
				<div className="flex flex-col sm:flex-row gap-3">
					...
					>
						{loading ? <LoadingSpinner size="small" /> : 'Calculate PO RD'}
					</button>
					<button
						type="button"
						onClick={handleReset}
