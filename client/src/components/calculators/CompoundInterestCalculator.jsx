 ...
 */
export default function CompoundInterestCalculator({ interestRates }) {
	const defaultRate = interestRates?.savingsAccount || 6.5;
	...
	const handleCalculate = async (e) => {
		...
		try {
			...
			const response = await calculatorApi.calculateCompoundInterest({
			...
			setError(err.message || 'Failed to calculate compound interest');
		} finally {
			setLoading(false);
		}
	};
	...
	const handleReset = () => {
		setFormData({ principal: '', rate: defaultRate, time: '', frequency: 4 });
		setResult(null);
		setError(null);
	};
		...
		<div className="space-y-6">
			...
			<div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-r">
				<div className="flex">
					...
					<div>
						<h3 className="text-sm font-semibold text-blue-900 mb-1">About Compound Interest</h3>
						<p className="text-sm text-blue-800">Compounding grows faster than simple interest. Choose quarterly (default), monthly, half-yearly or yearly.</p>
					</div>
				</div>
			</div>
			...
			<form onSubmit={handleCalculate} className="space-y-6">
				<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
					<div>
						<label className="block text-sm font-semibold text-gray-700 mb-2" htmlFor="principal">Principal Amount (?) *</label>
						<input
							id="principal"
							name="principal"
							type="number"
					...
					<div>
						<label className="block text-sm font-semibold text-gray-700 mb-2" htmlFor="time">Time (years) *</label>
						<input
							id="time"
							name="time"
							type="number"
					...
					<div>
						<label className="block text-sm font-semibold text-gray-700 mb-2" htmlFor="frequency">Compounding Frequency</label>
						<select
							id="frequency"
							name="frequency"
				...
				<div className="flex flex-col sm:flex-row gap-3">
					...
					>
						{loading ? <LoadingSpinner size="small" /> : 'Calculate Compound Interest'}
					</button>
					<button
						type="button"
						onClick={handleReset}
				...
				<div className="mt-8 bg-gradient-to-br from-emerald-50 to-teal-50 rounded-xl p-6 border-2 border-emerald-200">
					...
					<div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
						<div className="bg-white rounded-lg p-4 shadow-md">
							...
							<p className="text-2xl font-bold text-emerald-600">{formatCurrency(result.interest)}</p>
						</div>
						<div className="bg-white rounded-lg p-4 shadow-md">
						...
						<div className="bg-white rounded-lg p-4 shadow-md">
							...
							<p className="text-lg font-semibold">{result.frequency} times/year</p>
						</div>
					</div>
